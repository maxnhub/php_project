...
// перечисляем все поля из которых мы сделаем строку адреса
$addressFields = ['street', 'home', 'part', 'appt', 'floor'];
$address = '';
//перебираем все поля, которые у нас есть в POST и при свопадении склеиваем строку
...
// если пользователь есть, то увеличиваем ему кол-во заказов и передаём его id
if ($user) {
    $userId = $user['id'];
    ...
    $orderNumber = 1;
    // если пользователя нет, то мы его создаём
